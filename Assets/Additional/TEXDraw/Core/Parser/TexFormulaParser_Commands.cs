...
namespace TexDrawLib
{
    public partial class TexFormulaParser
    {
        ...
        static readonly HashSet<string> NotFamily = new HashSet<string>(new string[] {
            "not", "nnot", "hnot", "dnot", "unot", "onot", "vnot", "vnnot",
        });

        static readonly HashSet<string> HoldFamily = new HashSet<string>(new string[] {
            "hold", "vhold", "thold", "bhold", "lhold", "rhold",
        });

        static readonly HashSet<string> FracFamily = new HashSet<string>(new string[] {
        ...
        static readonly HashSet<string> TableFamily = new HashSet<string>(new string[] {
        ...
        public static readonly string[] commands = new string[]
        {
            ...
            "mclr", "clr", "bg", "vbg", "size", "text", "math", "style",
        ...
        private Atom ProcessCommand(TexFormula formula, string value, ref int position, string command)
        {
            ...
            switch (command)
            {
                ...
                case "text":
                    ...
                    return ParseFontStyle(value, ref position, idx, style);
                case "clr":
                case "mclr":
                case "color":
                    return Color(value, ref position, command == "color" ? 1 : (command == "clr" ? 0 : 2));
                case "bg":
                case "vbg":
                ...
                case "link":
                case "ulink":
                     return Link(value, ref position, command == "ulink");
                case "trs":
                case "mtrs":
                case "ltrs":
        ...
        static void holdP(string prefix, ref bool isVertical, ref TexAlignment align)
        {
            if (prefix.Length > 0)
            {
                switch (prefix[0])
        ...
        Atom Meta(TexFormula formula, string value, ref int position)
        {
        ...
        string ReadInsideBracket(string value, ref int position)
        {
            string r = null;
            if (value[position] == leftBracketChar)
            {
        ...
        FontStyle ParseStyle(string value, ref int position)
        {
            if (value[position] == leftBracketChar)
            {
                var str = ReadGroup(value, ref position, leftBracketChar, rightBracketChar);
                SkipWhiteSpace(value, ref position);
                return ParseFontStyle(str);
            }
            return TexUtility.FontStyleDefault;
        }

        Atom ParseFontStyle(string value, ref int position, int font = -2, FontStyle style = TexUtility.FontStyleDefault)
        {
        ...
        Atom Color(string value, ref int position, int mixmode)
        {
            string clr = ReadInsideBracket(value, ref position);
        ...
        Atom BgColor(string value, ref int position, bool margin)
        {
            string clr = ReadInsideBracket(value, ref position);
        ...
        Atom Size(string value, ref int position)
        {
        ...
        int ParseNotMode(char prefix, char second = '\0')
        {
            switch (prefix)
            {
                case 'n':
        ...
        Atom Not(string value, ref int position, int mode, bool margin)
        {
            string sz = ReadInsideBracket(value, ref position);
        ...
        Atom TRS(string value, ref int position, int mode)
        {
        ...
        Atom Frac(string value, ref int position, bool delim, TexAlignment nom, TexAlignment denom)
        {
            if (position == value.Length)
                return null;
