...
public class UniverseRunnerEditor : Editor
{
    ...
    {
        if(universe.simEnv == null || !File.Exists(simSettingsFilePath)) {
            universe.simEnv = ScriptableObject.CreateInstance<SimulationEnv>();
            SimulationEnv.WriteToFileSimuSettingsSaveData(universe.simEnv);
        }
        else {
    ...
    private void CreateUniverseRunnerSettingsEditor(SimulationEnv param)
    {
        ...
        if(GUI.changed)
        {
            string filepath = SimulationEnv.WriteToFileSimuSettingsSaveData(param);
