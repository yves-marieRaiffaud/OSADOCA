...
public struct OrbitalParamsSaveData
{
    ...
    public static OrbitalParams LoadObjectFromJSON(string filepath)
    {
        ...
        output.orbitDefType = (OrbitalTypes.orbitDefinitionType) MathsOps.TryParse_String_To_Int(loadedData.orbitDefTypeInt, 0);
        output.bodyPosType = (OrbitalTypes.bodyPositionType) MathsOps.TryParse_String_To_Int(loadedData.bodyPosTypeInt, 0);
        output.orbParamsUnits = (OrbitalTypes.orbitalParamsUnits) MathsOps.TryParse_String_To_Int(loadedData.orbParamsUnitsInt, 0);
        output.orbitRealPredType = (OrbitalTypes.typeOfOrbit) MathsOps.TryParse_String_To_Int(loadedData.orbitRealPredTypeInt, 0);
        output.selectedVectorsDir = (OrbitalTypes.typeOfVectorDir) MathsOps.TryParse_String_To_Int(loadedData.selectedVectordDirInt, 0);

        output.drawOrbit = MathsOps.TryParse_String_To_Bool(loadedData.drawOrbitInt, false);
        output.drawDirections = MathsOps.TryParse_String_To_Bool(loadedData.drawDirectionsInt, false);

        output.orbitDrawingResolution = MathsOps.TryParse_String_To_Int(loadedData.orbitDrawingResolutionInt, 300);
        MathsOps.ParseStringToDouble(loadedData.raDouble, out output.ra);
        MathsOps.ParseStringToDouble(loadedData.rpDouble, out output.rp);
        MathsOps.ParseStringToDouble(loadedData.pDouble, out output.p);
        MathsOps.ParseStringToDouble(loadedData.eDouble, out output.e);
        MathsOps.ParseStringToDouble(loadedData.iDouble, out output.i);
        MathsOps.ParseStringToDouble(loadedData.lAscNDouble, out output.lAscN);
        MathsOps.ParseStringToDouble(loadedData.omegaDouble, out output.omega);
        MathsOps.ParseStringToDouble(loadedData.nuDouble, out output.nu);
