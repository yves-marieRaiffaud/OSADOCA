...
public static class Kepler
{
    ...
    public static Vector3d GravitationalAcc<T1>(CelestialBody pullingBody, T1 orbitingBody, bool saveAccToOrbitingBodyParam)
    ...
    where T1: FlyingObjCommonParams
    {
        double dstPow3 = Mathd.Pow(r.magnitude*UniCsts.km2m, 3); // m^3
        double mu = pullingBody.settings.planetBaseParamsDict[CelestialBodyParamsBase.planetaryParams.mu.ToString()].value;
        //Vector3d acc =  - Mathd.Pow(10,7) * mu * r / dstPow3; // m.s-2
        Vector3d acc =  - UniCsts.µExponent * mu * r*UniCsts.km2m / dstPow3; // m.s-2
