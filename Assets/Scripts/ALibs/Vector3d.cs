...
namespace Mathd_Lib {
    ...
    public struct Vector3d {
        ...
        public static bool IsValid(Vector3d a) {
            if(double.IsNaN(a.x) || double.IsNaN(a.y) || double.IsNaN(a.z) ||
            double.IsInfinity(a.x) || double.IsInfinity(a.y) || double.IsInfinity(a.z)) {
                return false;
            }
            return true;
        }
        ...
        public static bool IsValidAndNoneZero(Vector3d a) {
            if(IsValid(a) && !IsZero(a)) { return true; }
            else{ return false; }
        }

        public static bool IsInvalindOrZero(Vector3d a) {
            if(!IsValid(a) || IsZero(a)) { return true; }
            else { return false; }
        }
    }
